...
public class ElasticsearchClient extends BaseClient {
    ...
    public static Map<String, Object> connectionTest(String serviceName, Map<String, String> configs) {
        ...
        if (connectivityStatus) {
            ...
            BaseClient.generateResponseDataMap(connectivityStatus, failureMsg, failureMsg + DEFAULT_ERROR_MESSAGE, null, null, responseData);
        }
    ...
    private static ClientResponse getClientResponse(String url, Client client, String userName) {
        ...
        if (response != null) {
            ...
            if (response.getStatus() != HttpStatus.SC_OK) {
                LOG.warn("getClientResponse():response.getStatus()= " + response.getStatus() + " for URL " + url + ", failed to get elasticsearch resource list, response= " + response.getEntity(String.class));
            }
        }
        return response;
    }
