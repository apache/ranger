...
if [[ $flag == true ]]; then
  if [[ "${RUN_TESTS}" == "1" ]]; then
    ...
    if [[ "${TEST_TYPE}" == "all" ]]; then
      pytest -vs kms/ --html=report_kms.html
      pytest -vs xuserrest/ --html=report_admin.html
      pytest -vs servicerest/ --html=report_usersync.html
    else
      #pytest -vs ${TEST_TYPE}/ --html=report_${TEST_TYPE}.html
      pytest -vs kms/ --html=report_kms.html
      pytest -vs xuserrest/ --html=report_admin.html
      pytest -vs servicerest/ --html=report_usersync.html
    fi
