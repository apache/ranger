...
DECLARE
    ...
    v_plan_json CLOB := '{"plugins":{},"buffer":{"partitions":[1,2,3,4,5,6,7,8,9]}}';
...
BEGIN
    ...
    SELECT count(*) INTO t_count FROM user_tables WHERE table_name = 'X_PORTAL_USER';
    IF (t_count > 0) THEN
    ...
    IF (v_cfg_count = 0) THEN
        ...
        COMMIT;
    END IF;
    SELECT count(*) INTO v_cfg_count FROM x_audit_config WHERE cfg_name = 'topic';
    IF (v_cfg_count = 0) THEN
        sql_stmt := 'INSERT INTO x_audit_config (id, create_time, update_time, cfg_name, cfg_value, version) VALUES (X_AUDIT_CONFIG_SEQ.nextval, sys_extract_utc(systimestamp), sys_extract_utc(systimestamp), :1, :2, 1)';
        EXECUTE IMMEDIATE sql_stmt USING 'topic', 'ranger_audits';
        COMMIT;
    END IF;
    SELECT count(*) INTO v_cfg_count FROM x_audit_config WHERE cfg_name = 'RangerAuditPartitionPlan';
    IF (v_cfg_count = 0) THEN
        sql_stmt := 'INSERT INTO x_audit_config (id, create_time, update_time, cfg_name, cfg_value, version) VALUES (X_AUDIT_CONFIG_SEQ.nextval, sys_extract_utc(systimestamp), sys_extract_utc(systimestamp), :1, :2, 1)';
        EXECUTE IMMEDIATE sql_stmt USING 'RangerAuditPartitionPlan', v_plan_json;
        COMMIT;
    END IF;
END;
/
