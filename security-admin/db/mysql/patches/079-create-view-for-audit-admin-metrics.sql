...
drop procedure if exists create_index_for_x_trx_log_v2_metrics;

delimiter ;;
create procedure create_index_for_x_trx_log_v2_metrics() begin
if exists (SELECT * FROM INFORMATION_SCHEMA.STATISTICS WHERE table_schema=DATABASE() AND table_name='x_trx_log_v2') then
    if not exists (SELECT * FROM INFORMATION_SCHEMA.STATISTICS WHERE table_schema=DATABASE() AND table_name='x_trx_log_v2' AND index_name='x_trx_log_v2_IDX_metrics') then
...
delimiter ;
call create_index_for_x_trx_log_v2_metrics();

DROP VIEW IF EXISTS vx_audit_admin_metrics_by_days;

CREATE OR REPLACE VIEW vx_audit_admin_metrics_by_days AS
SELECT
    class_type,
    action,
