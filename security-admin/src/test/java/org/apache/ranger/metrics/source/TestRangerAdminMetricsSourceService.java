...
public class TestRangerAdminMetricsSourceService {
    ...
    public void testRefresh_WithValidMetrics() {
        ...
        verify(spyService).addMetricEntries(EXPECTED_METRIC_PREFIX, mockMetrics);
    ...
    public void testRefresh_WithEmptyMetrics() {
        ...
        when(rangerMetricsFetcher.getRangerServiceMetrics()).thenReturn(new HashMap<>());

        spyService.refresh();

        verify(spyService).addMetricEntries(EXPECTED_METRIC_PREFIX, emptyMetrics);
        verify(rangerMetricsFetcher).getRangerServiceMetrics();
    }
