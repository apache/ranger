...
public class TestPolicyRefUpdater {
    ...
    public void testCreateNewPolMappingForRefTable_InvalidResourceDef() throws Exception {
        ...
        try {
            updater.createNewPolMappingForRefTable(policy, xPolicy, xSvc, false, false);
    ...
    public void testCleanupRefTablesForUpdate_SelectivePrincipalCleanup() {
        ...
        Assert.assertTrue(updater.cleanupRefTablesForUpdate(policyId, policyUsers, policyRoles, policyGroups));

        verify(userDao).deletePolicyRefUserByIds(Collections.singletonList(4L));
        Assert.assertEquals(Collections.singleton("carol"), policyUsers);

        verify(roleDao).deletePolicyRefRoleByIds(Collections.singletonList(12L));
        Assert.assertEquals(new HashSet<>(Arrays.asList("roleB", "roleNew")), policyRoles);

        verify(groupDao).deletePolicyRefGroupByIds(Collections.singletonList(22L));
        Assert.assertEquals(new HashSet<>(Arrays.asList("grp2", "grpNew")), policyGroups);
    ...
    public void testCleanupRefTables() {
        ...
        RangerPolicy p = new RangerPolicy();
        p.setId(77L);
        Assert.assertEquals(true, updater.cleanupRefTables(p));
        Assert.assertEquals(false, updater.cleanupRefTables(null));
    }
