...
//				xServiceList);
	...
	public void test20aCreateServiceEncryptsAllServiceDefTypePasswordConfigs() throws Exception {
		...
		Map<String, String> persisted = new HashMap<String, String>();
		for (XXServiceConfigMap m : captor.getAllValues()) {
			persisted.put(m.getConfigkey(), m.getConfigvalue());
		}

		Assert.assertNotEquals(plainKeystorePwd, persisted.get(keyKeystorePwd));
		Assert.assertNotEquals(plainTruststorePwd, persisted.get(keyTruststorePwd));
		Assert.assertTrue(persisted.get(keyKeystorePwd).contains(","));
		Assert.assertTrue(persisted.get(keyTruststorePwd).contains(","));
		Assert.assertEquals(plainUrl, persisted.get(keyUrl));
	}
	...
	public void test20aLiteralPasswordKeyStillEncryptedOnCreate() throws Exception {
		...
		Mockito.when(daoManager.getXXServiceConfigDef()).thenReturn(xServiceConfigDefDao);
		Mockito.when(xServiceConfigDefDao.findByServiceDefName("hive")).thenReturn(Collections.<XXServiceConfigDef>emptyList());
		...
		Assert.assertNotEquals(plainPassword, persisted.get(keyPassword));
		Assert.assertTrue(persisted.get(keyPassword).contains(","));
		Assert.assertEquals(plainUrl, persisted.get(keyUrl));
	}
	...
	public void test20bUpdateServicePreservesPerKeyEncryptedValueOnHiddenSentinel() throws Exception {
		...
		Assert.assertNotEquals(oldKeystoreEncrypted, oldTruststoreEncrypted);
		...
		existing.setGuid("1427365526516_835_0");
		...
		Assert.assertEquals(oldKeystoreEncrypted, persisted.get(keyKeystorePwd));
		Assert.assertEquals(oldTruststoreEncrypted, persisted.get(keyTruststorePwd));
		Assert.assertNotEquals(oldTruststoreEncrypted, persisted.get(keyKeystorePwd));
		Assert.assertNotEquals(oldKeystoreEncrypted, persisted.get(keyTruststorePwd));
	}
	...
	public void test20cUpdateServiceReEncryptsChangedKeyAndPreservesOtherOnHiddenSentinel() throws Exception {
		...
		Assert.assertNotEquals(oldKeystoreEncrypted, persisted.get(keyKeystorePwd));
		Assert.assertNotEquals(newKeystorePwd, persisted.get(keyKeystorePwd));
		Assert.assertEquals(oldTruststoreEncrypted, persisted.get(keyTruststorePwd));
	}
