...
public class TestServiceREST {
	...
	public void testDeletePolicy_delegatedAdminWithoutModifyAccessIsForbidden() throws Exception {
		...
		Mockito.when(userMgr.getGroupsForUser(Mockito.anyString())).thenReturn(new HashSet<String>());
		...
		Mockito.when(policyAdmin.getRolesFromUserAndGroups(Mockito.anyString(), Mockito.anySet())).thenReturn(new HashSet<String>());
		...
		try {
			spy.deletePolicy(Id);
			Assert.fail("deletePolicy() must throw when the modify check fails");
...
*/
	...
	@Test
	public void test155DownloadBlockedWhenUnauthenticatedAndFlagFalse() throws Exception {
		HttpServletRequest request = Mockito.mock(HttpServletRequest.class);
		...
		Assert.assertThrows(WebApplicationException.class, () -> serviceREST.getServicePoliciesIfUpdated("HDFS_1",
				1L, 0L, "1", "", "", false, capabilityVector, request));
	}

	@Test
	public void test156GrantBlockedWhenUnauthenticatedAndFlagFalse() throws Exception {
		HttpServletRequest request = Mockito.mock(HttpServletRequest.class);
		...
		Assert.assertThrows(WebApplicationException.class, () -> serviceREST.grantAccess("HDFS_1", grantRequest, request));
	}
}
