...
public class TestPublicAPIsv2 {
    ...
    public void test69addUsersAndGroups() throws Exception {
        ...
        Mockito.when(roleREST.addUsersAndGroups(eq(Id), eq(users), eq(groups), eq(isAdmin), isNull())).thenReturn(role);

        RangerRole result = publicAPIsv2.addUsersAndGroups(Id, null, users, groups, isAdmin, request);

        Assertions.assertNotNull(result);
        Assertions.assertEquals(result.getId(), Id);

        Mockito.verify(roleREST).addUsersAndGroups(eq(Id), eq(users), eq(groups), eq(isAdmin), isNull());
    }
    ...
    public void test70removeUsersAndGroups() throws Exception {
        ...
        Mockito.when(roleREST.removeUsersAndGroups(eq(Id), eq(users), eq(groups), isNull())).thenReturn(role);

        RangerRole result = publicAPIsv2.removeUsersAndGroups(Id, null, users, groups, request);

        Assertions.assertNotNull(result);
        Assertions.assertEquals(result.getId(), Id);

        Mockito.verify(roleREST).removeUsersAndGroups(eq(Id), eq(users), eq(groups), isNull());
    }
    ...
    public void test71removeAdminFromUsersAndGroups() throws Exception {
        ...
        Mockito.when(roleREST.removeAdminFromUsersAndGroups(eq(Id), eq(users), eq(groups), isNull())).thenReturn(role);

        RangerRole result = publicAPIsv2.removeAdminFromUsersAndGroups(Id, null, users, groups, request);

        Assertions.assertNotNull(result);
        Assertions.assertEquals(result.getId(), Id);

        Mockito.verify(roleREST).removeAdminFromUsersAndGroups(eq(Id), eq(users), eq(groups), isNull());
    }
