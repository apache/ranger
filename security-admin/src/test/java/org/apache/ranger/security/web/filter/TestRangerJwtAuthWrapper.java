...
public class TestRangerJwtAuthWrapper {
    ...
    @Test
    public void testDoFilter_doesNotRedirectForNonBrowserAgent() throws IOException, ServletException {
        System.setProperty("ranger.default.browser-useragents", "Mozilla,Chrome,Opera");
        SecurityContextHolder.clearContext();
        PropertiesUtil.getPropertiesMap().put("ranger.sso.enabled", "false");
        ...
        RangerJwtAuthWrapper wrapper = Mockito.spy(new RangerJwtAuthWrapper());
        wrapper.initialize();
        wrapper.rangerJwtAuthFilter = Mockito.mock(RangerJwtAuthFilter.class);

        HttpServletRequest req = Mockito.mock(HttpServletRequest.class);
        HttpServletResponse res = Mockito.mock(HttpServletResponse.class);
        FilterChain chain = Mockito.mock(FilterChain.class);

        // No Authorization header -> JWT auth skipped; ensure no redirect and chain proceeds
        wrapper.doFilter(req, res, chain);

        verify(res, never()).sendRedirect(any(String.class));
        verify(chain, times(1)).doFilter(req, res);
    }
}
