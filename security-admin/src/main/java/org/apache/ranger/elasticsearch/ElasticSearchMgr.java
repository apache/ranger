		...
		if (client == null) {
			synchronized (ElasticSearchAuditDestination.class) {
				if (client == null) {
					...
					logger.warn(String.format("Initializing ElasticSearch (URLs: %s; Protocol: %s; Port: %s)", urls, protocol, port));
					if (urls != null) {
						urls = urls.trim();
					}
					if (urls != null && urls.equalsIgnoreCase("NONE")) {
						logger.warn(String.format("Clearing URI config value: %s", urls));
						urls = null;
					}
					...
					try {
						...
						logger.fatal("Can't connect to ElasticSearch server" + "." + " urls=" + urls, t);
					}
				}
			}
		}
