...
public class PatchForKafkaServiceDefUpdate_J10033 extends BaseLoader {
    ...
    private static final List<String> DEFAULT_POLICY_GROUP = new ArrayList<>(Collections.singletonList("public"));
    ...
    private void updateKafkaServiceDef() {
        try {
            RangerServiceDef embeddedKafkaServiceDef = EmbeddedServiceDefsUtil.instance().getEmbeddedServiceDef(SERVICEDBSTORE_SERVICEDEFBYNAME_KAFKA_NAME);

            if (embeddedKafkaServiceDef != null) {
                XXServiceDef        xXServiceDefObj            = daoMgr.getXXServiceDef().findByName(SERVICEDBSTORE_SERVICEDEFBYNAME_KAFKA_NAME);
                ...
                RangerServiceDef dbKafkaServiceDef = svcDBStore.getServiceDefByName(SERVICEDBSTORE_SERVICEDEFBYNAME_KAFKA_NAME);

                if (dbKafkaServiceDef != null) {
                    List<RangerServiceDef.RangerResourceDef>   embeddedKafkaResourceDefs = embeddedKafkaServiceDef.getResources();
                    List<RangerServiceDef.RangerAccessTypeDef> embeddedKafkaAccessTypes  = embeddedKafkaServiceDef.getAccessTypes();
                    ...
                    RangerServiceDef ret = svcStore.updateServiceDef(dbKafkaServiceDef);

                    if (ret == null) {
                        logger.error("Error while updating {} service-def", SERVICEDBSTORE_SERVICEDEFBYNAME_KAFKA_NAME);
            ...
            logger.error("Error while updating {} service-def", SERVICEDBSTORE_SERVICEDEFBYNAME_KAFKA_NAME, e);
        }
    }
    ...
    private void createDefaultPolicyForNewResources() {
        ...
        for (XXService xxService : xxServices) {
            ...
            try {
                List<RangerPolicy> rangerpolicies = svcDBStore.getPoliciesByResourceSignature(xxService.getName(), rangerPolicy.getResourceSignature(), true);

                if (CollectionUtils.isNotEmpty(rangerpolicies)) {
                    for (RangerPolicy rPolicy : rangerpolicies) {
                        if (rPolicy != null) {
                            logger.debug("print Policy: {}", rPolicy);
                            logger.debug("policy found with resource {} and ResourceSignature {} service name:{}", rPolicy.getResources(), rPolicy.getResourceSignature(), rPolicy.getService());
                ...
                logger.error(" Error while getting policy using Resource Signature, Service Name and policy enabled flag", ex);
            }
            ...
            if (!policyExist) {
                ...
                for (int i = 0; i < DEFAULT_POLICY_USERS.size(); i++) {
                    ...
                    if (xxUser == null) {
                        logger.info("{} user is not found, adding user: {}", user, user);
    ...
    private RangerPolicy getRangerPolicy(String newResource, XXPortalUser xxPortalUser, XXService xxService) {
        ...
        policyResource.put(CONSUMERGROUP_RESOURCE_NAME, rangerPolicyResource);
    ...
    private List<String> getAccessTypes() {
        return Arrays.asList("consume", "describe", "delete");
    }
