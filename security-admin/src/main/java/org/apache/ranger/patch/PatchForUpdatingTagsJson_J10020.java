            ...
            if (this.daoMgr != null && this.xService != null) {
                if (this.txTemplate == null) {
                    ...
                    if (!initializeTagCache(xService)) {
                        logger.error("Failed to get tags for service:[" + xService.getName() + "] in the same thread and using an existing transaction");
                    }
                } else {
                ...
                Boolean result = txTemplate.execute(new TransactionCallback<Boolean>() {
                    ...
                    public Boolean doInTransaction(TransactionStatus status) {
                        ...
                        if (!ret) {
                            ...
                            logger.error("Failed to get tags for service:[" + xService.getName() + "] in a new transaction");
                        }
                        return ret;
                    }
                });
