...
public class RangerTrxLogV2Service {
    ...
    public List<VXTrxLogV2> findByTransactionId(String transactionId) {
        ...
        if (trxLogsV2 != null && !trxLogsV2.isEmpty()) {
            ...
            UserSessionBase session = ContextUtil.getCurrentUserSession();
            if (session != null && (session.isKeyAdmin() || session.isAuditKeyAdmin())) {
                ret = trxLogsV2.stream().filter(xTrxLog -> getValidTrxLogsForKeyAdminAndAuditor(xTrxLog)).map(xTrxLog -> toViewObject(xTrxLog, uidNameCache)).collect(Collectors.toList());
            } else {
    ...
    public String applyKeyAdminAccessFilters(Map<String, Object> parameters) {
        ...
        if (xxServiceDef != null) {
            List<Long> parentObjectId = daoManager.getXXService().findByServiceDefId(xxServiceDef.getId()).stream().map(XXService::getId).collect(Collectors.toList());
