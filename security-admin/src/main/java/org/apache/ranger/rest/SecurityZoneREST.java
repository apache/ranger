...
public class SecurityZoneREST {
    ...
    public RangerSecurityZone createSecurityZone(RangerSecurityZone securityZone) {
        ...
        try {
            ...
            LOG.error("createSecurityZone({}) failed", securityZone, excp);
    ...
    public RangerSecurityZone updateSecurityZone(@PathParam("id") Long zoneId, RangerSecurityZone securityZone) {
        ...
        try {
            ...
            LOG.error("updateSecurityZone({}) failed", securityZone, excp);
    ...
    public void deleteSecurityZone(@PathParam("name") String zoneName) {
        ...
        try {
            ...
            LOG.error("deleteSecurityZone({}) failed", zoneName, excp);
    ...
    public void deleteSecurityZone(@PathParam("id") Long zoneId) {
        ...
        try {
            ...
            LOG.error("deleteSecurityZone({}) failed", zoneId, excp);
            ...
            throw restErrorUtil.createRESTException("Data Not Found for given Id", MessageEnums.DATA_NOT_FOUND, zoneId, null, "readResource : No Object found with given id.");
        }
    ...
    public RangerSecurityZone getSecurityZone(@PathParam("name") String zoneName) {
        ...
        try {
            ...
            LOG.error("getSecurityZone({}) failed", zoneName, excp);
    ...
    public RangerSecurityZone getSecurityZone(@PathParam("id") Long id) {
        ...
        try {
            ...
            LOG.error("getSecurityZone({}) failed", id, excp);
    ...
    public Collection<String> getZoneNamesForResource(@PathParam("serviceName") String serviceName, @Context HttpServletRequest request) {
        ...
        if (!serviceRest.isServiceAdmin(serviceName)) {
            throw restErrorUtil.createRESTException(HttpServletResponse.SC_FORBIDDEN, "User '" + bizUtil.getCurrentUserLoginId() + "' does not have privilege", true);
        }
        ...
        LOG.debug("<== SecurityZoneREST.getZoneNamesForResource(serviceName={}):{}", serviceName, ret);
    ...
    private void ensureUserAllowOperationOnServiceForZone(RangerSecurityZone securityZone) {
        if (!bizUtil.isAdmin()) {
            String             userName             = bizUtil.getCurrentUserLoginId();
            RangerSecurityZone existingSecurityZone;
            ...
            if (existingSecurityZone != null) {
                ...
                if (!serviceMgr.isZoneAdmin(existingSecurityZone.getName())) {
                    if (!Objects.equals(securityZone.getAdminUserGroups(), existingSecurityZone.getAdminUserGroups())) {
                        ...
                        throwRestError("User : " + userName + " is not allowed to edit zone Admin Roles of zone : " + existingSecurityZone.getName());
                        ...
                        throwRestError("User : " + userName + " is not allowed to edit zone Audit Roles of zone : " + existingSecurityZone.getName());
                    }
                }
                ...
                List<String> addRmvTagSvc  = new ArrayList<>();
    ...
    private void ensureAdminAccess(RangerSecurityZone securityZone) {
        if (!bizUtil.isAdmin()) {
            ...
            throw restErrorUtil.createRESTException("Ranger Security Zone is not accessible for user '" + userName + "'.", MessageEnums.OPER_NO_PERMISSION);
        } else {
    ...
    private void removeEmptyEntries(RangerSecurityZone securityZone) {
        ...
        if (serviceResouceMap != null) {
            ...
            for (Map.Entry<String, RangerSecurityZoneService> serviceResouceMapEntry : serviceResouceMapEntries) {
