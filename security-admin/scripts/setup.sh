...
FF_ENAB_OZONE_ACTION_MATCHES_CONDITION=$(get_prop_or_default 'FF_ENAB_OZONE_ACTION_MATCHES_CONDITION' $PROPFILE 'false')
...
update_properties() {
	...
	ff_enab_ozone_action_matches_condition=$(echo "${FF_ENAB_OZONE_ACTION_MATCHES_CONDITION}" | tr '[:upper:]' '[:lower:]')
	if [ "${ff_enab_ozone_action_matches_condition}" != "true" ]
	then
		ff_enab_ozone_action_matches_condition=false
	fi
	propertyName=ranger.servicedef.ozone.enableActionMatcherInPoliciesCondition
	newPropertyValue="${ff_enab_ozone_action_matches_condition}"
	updatePropertyToFilePy $propertyName "${newPropertyValue}" $to_file_ranger
