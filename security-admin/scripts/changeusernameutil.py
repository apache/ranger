...
def main(argv):
        ...
        if userName != "" and oldPassword != "" and newUserName != "":
                ...
                get_java_cmd = "%s -Dlogdir=%s -Dlogback.configurationFile=db_patch.log4j.xml -cp %s org.apache.ranger.patch.cliutil.%s %s %s %s"%(JAVA_BIN,ranger_log,path,'ChangeUserNameUtil',userName,oldPassword,newUserName)
                if os_name == "LINUX":
