...
# ========================================
# Ranger Audit Consumer Solr Start Script
# ========================================
# This script starts the Solr consumer service
# The service consumes audit events from Kafka and indexes them to Solr
...
# Find the WAR file
WAR_FILE=$(find "${AUDIT_CONSUMER_HOME_DIR}" -name "ranger-audit-consumer-solr*.war" | head -1)
...
WEBAPP_DIR="${AUDIT_CONSUMER_HOME_DIR}/webapp/ranger-audit-consumer-solr"
...
PID_FILE="${AUDIT_CONSUMER_LOG_DIR}/ranger-audit-consumer-solr.pid"
if [ -f "${PID_FILE}" ]; then
  OLD_PID=$(cat "${PID_FILE}")
  if kill -0 "$OLD_PID" 2>/dev/null; then
    echo "[WARNING] Ranger Audit Consumer - Solr is already running (PID: ${OLD_PID})"
    echo "[INFO] Use stop-consumer-solr.sh to stop it first"
    exit 1
  else
...
echo "[INFO] ✓ Ranger Audit Consumer - Solr started successfully"
echo "[INFO] PID: ${PID}"
echo "[INFO] Log file: ${AUDIT_CONSUMER_LOG_DIR}/ranger-audit-consumer-solr.log"
echo "[INFO] Catalina out: ${AUDIT_CONSUMER_LOG_DIR}/catalina.out"
echo "[INFO] Health check: http://localhost:7091/api/health"
echo ""
echo "To monitor logs: tail -f ${AUDIT_CONSUMER_LOG_DIR}/ranger-audit-consumer-solr.log"
echo "To stop service: ${SCRIPT_DIR}/stop-consumer-solr.sh"
