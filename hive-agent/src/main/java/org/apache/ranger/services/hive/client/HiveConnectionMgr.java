...
public class HiveConnectionMgr {
    ...
    public HiveConnectionMgr() {
        hiveConnectionCache  = new ConcurrentHashMap<>();
    ...
    public HiveClient getHiveConnection(final String serviceName, final String serviceType, final Map<String, String> configs) {
        ...
        if (serviceType != null) {
            ...
            if (hiveClient == null) {
                if (configs != null) {
                    final Callable<HiveClient> connectHive = () -> new HiveClient(serviceName, configs);

                    try {
                        hiveClient = TimedEventUtil.timedTask(connectHive, 5, TimeUnit.SECONDS);
                    } catch (Exception e) {
                        LOG.error("Error connecting hive repository : {} using config : {}", serviceName, configs, e);
                    }
