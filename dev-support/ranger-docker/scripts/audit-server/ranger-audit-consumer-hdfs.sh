...
AUDIT_CONSUMER_LOG_DIR="${AUDIT_CONSUMER_LOG_DIR:-/var/log/ranger/audit-consumer-hdfs}"
...
HDFS_NAMENODE="${HDFS_NAMENODE:-ranger-hadoop.rangernw:9000}"
check_tcp_port "HDFS NameNode" "${HDFS_NAMENODE}" 60
...
# Point to krb5.conf and Hadoop configs for Kerberos
...
PID=$!
echo $PID > ${AUDIT_CONSUMER_LOG_DIR}/ranger-audit-consumer-hdfs.pid

echo "[INFO] Ranger Audit Consumer - HDFS started with PID: $PID"
