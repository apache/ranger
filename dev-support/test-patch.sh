...
printUsage() {
  ...
  echo "  defect-number is a JIRA defect number (e.g. 'RANGER-1234') to test (Jenkins only)"
  echo
  echo "Options:"
...
checkAuthor () {
  ...
  if [[ $authorTags != 0 ]] ; then
    ...
    {color:red}-1 @author{color}.  The patch appears to contain $authorTags @author tags which the Ranger community has agreed to not allow in code contributions."
    return 1
  fi
  JIRA_COMMENT="$JIRA_COMMENT
