...
public class RangerAzureKeyVaultKeyGenerator implements RangerKMSMKI {
    ...
    public boolean generateMasterKey(String password) throws Exception {
        ...
        try {
            ...
            throw new Exception("Error while getting existing master key from Azure.  Master Key Name : "
                            + azureMasterKey + " . Key Vault URL : " + keyVaultURL + " . Error : " + ex.getMessage());
        }
        if (masterKeyBundle == null) {
            try {
                ...
                CreateKeyRequest createKeyRequest = new CreateKeyRequest.Builder(keyVaultURL, azureMasterKey, keyType)
                        .withAttributes(masterKeyattribute).build();
