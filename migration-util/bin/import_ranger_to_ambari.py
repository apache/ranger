      ...
      if create_ranger_service_components_in_ambari('RANGER_ADMIN'):
        ...
        if register_ranger_admin_host_in_ambari():
          ...
          if add_advanced_ranger_configurations(1,ranger_admin_properties_from_file):
            print('Ranger advanced configurations added successfully in Ambari, kindly run Ranger-Usersync to complete Ranger service install')                        
          else:
...
def create_ranger_service_in_ambari():
  ...
  if request_result is not None:
    ...
    if (response_code == 201 and response is not None):
      ...
      print('Ranger service is alread created in Ambari.')
      return False
    else:
...
def add_advanced_ranger_configurations(add_admin_or_usersync, ranger_service_properties_from_file):
  ...
  if(add_admin_or_usersync == 1):
    ranger_service_properties_from_file = get_db_flavour(ranger_service_properties_from_file)
    ...
    advanced_admin_properties['db_user'] = ranger_service_properties_from_file['db_user']
    advanced_admin_properties['db_password'] = ranger_service_properties_from_file['db_password']
    advanced_admin_properties['audit_db_name'] = ranger_service_properties_from_file['audit_db_name']
    advanced_admin_properties['audit_db_user'] = ranger_service_properties_from_file['audit_db_user']
    advanced_admin_properties['audit_db_password'] = ranger_service_properties_from_file['audit_db_password']
    ...
    advanced_admin_properties['authentication_method'] = ranger_service_properties_from_file['authentication_method']
    advanced_admin_properties['remoteLoginEnabled'] = ranger_service_properties_from_file['remoteLoginEnabled']
    advanced_admin_properties['authServiceHostName'] = ranger_service_properties_from_file['authServiceHostName']
    advanced_admin_properties['authServicePort'] = ranger_service_properties_from_file['authServicePort']
    ...
    advanced_ranger_site_properties['HTTP_ENABLED'] = ranger_service_properties_from_file['policymgr_http_enabled']
  ...
  if(add_admin_or_usersync == 2):
    ...
    ranger_config_data = '[{"Clusters":{"desired_config":[{"type":"usersync-properties","tag":"' + str(date_time_stamp) + '","properties":' + json.dumps(advanced_user_sync_properties) + ', "properties_attributes": {"final": "false"}}]}}]'
...
def get_db_flavour(ranger_admin_properties_from_file):
  print('inside get_db_flavour getting db flavor, library and command invoker')
  install_properties_db_flavour = ranger_admin_properties_from_file['DB_FLAVOR']
  print( 'install_properties_db_flavour = ' + install_properties_db_flavour)
  ...
  if('oracle'.lower() in xa_system_properties_db_dialect and 'oracle'.lower() in xa_system_properties_url.lower()):
...
def call_ranger_installed():
  ...
  ranger_state_change_request = '{"RequestInfo":{"context":"Install Ranger Service","operation_level":{"level":"CLUSTER","cluster_name":"' + str(cluster_name) + '"}},"Body":{"ServiceInfo":{"state":"INSTALLED"}}}'
      ...
      if response_status != 'Installed':
        ...
        while flag_ranger_installed:
          ...
          if request_status_result is not None:
            ...
            if  (response_code != 200 and 'FAILED' not in response_status):
              print('Ranger install failed in ambari.')
              flag_ranger_installed =  False
            if (response_code == 200 and 'PENDING' in response_status):
...
def get_ranger_usersync_install_properties():
  ...
  ranger_usersync_properties_from_file = None
  ...
  if flag_ranger_usersync_present:
    ...
    ranger_usersync_properties_from_file = import_properties_from_file("ranger_usersync_install.properties")
    if ranger_usersync_properties_from_file is not None and len(ranger_usersync_properties_from_file) != 0:
...
def get_ranger_admin_install_properties():
  ...
  ranger_admin_properties_from_file = None
  ...
  if flag_hadoop_present :
    ...
    if flag_ranger_admin_present:
      ...
      if os.path.isfile("ranger_admin_install.properties"):
        ...
        if ranger_admin_properties_from_file is not None and len(ranger_admin_properties_from_file) != 0:
...
def port_hdfs_plugin_to_ambari():
  ...
  if flag_hdfs_plugin_installed and hdfs_plugin_install_properties is not None and hdfs_site_xml_properties is not None:
    ...
    advanced_ranger_hdfs_plugin_properties['XAAUDIT.HDFS.DESTINTATION_ROLLOVER_INTERVAL_SECONDS'] = hdfs_plugin_install_properties['xasecure.audit.hdfs.config.destination.rollover.interval.seconds']
    advanced_ranger_hdfs_plugin_properties['XAAUDIT.HDFS.DESTINTATION_OPEN_RETRY_INTERVAL_SECONDS'] = hdfs_plugin_install_properties['xasecure.audit.hdfs.config.destination.open.retry.interval.seconds']
    ...
    advanced_ranger_hdfs_plugin_properties['XAAUDIT.HDFS.LOCAL_BUFFER_ROLLOVER_INTERVAL_SECONDS'] = hdfs_plugin_install_properties['xasecure.audit.hdfs.config.local.buffer.rollover.interval.seconds']
    ...
    date_time_stamp = getDateTimeNow()
    plugin_configuration_data = '[{"Clusters":{"desired_config":[{"type":"hdfs-site","tag":"' + str(date_time_stamp) + '","properties":' + str(json.dumps(hdfs_site_xml_properties)) + ', "properties_attributes": {"final": "false"}},{"type": "ranger-hdfs-plugin-properties", "tag": "' + str(date_time_stamp) + '", "properties":'+json.dumps(advanced_ranger_hdfs_plugin_properties)+',"properties_attributes": {"final": "false"}}]}}]'
...
def port_ranger_hive_plugin_to_ambari():
  ...
  if flag_hive_plugin_installed and hive_plugin_install_properties is not None and hive_server2_xml_properties is not None:
    ...
    advanced_ranger_hive_plugin_properties['XAAUDIT.HDFS.DESTINTATION_ROLLOVER_INTERVAL_SECONDS'] = hive_plugin_install_properties['xasecure.audit.hdfs.config.destination.rollover.interval.seconds']
    advanced_ranger_hive_plugin_properties['XAAUDIT.HDFS.DESTINTATION_OPEN_RETRY_INTERVAL_SECONDS'] = hive_plugin_install_properties['xasecure.audit.hdfs.config.destination.open.retry.interval.seconds']
    ...
    advanced_ranger_hive_plugin_properties['XAAUDIT.HDFS.LOCAL_BUFFER_ROLLOVER_INTERVAL_SECONDS'] = hive_plugin_install_properties['xasecure.audit.hdfs.config.local.buffer.rollover.interval.seconds']
    ...
    advanced_ranger_hive_plugin_properties['UPDATE_XAPOLICIES_ON_GRANT_REVOKE'] = hive_plugin_install_properties['UPDATE_XAPOLICIES_ON_GRANT_REVOKE']


    date_time_stamp = getDateTimeNow()
    plugin_configuration_data = '[{"Clusters":{"desired_config":[{"type":"hiveserver2-site","tag":"' + str(date_time_stamp) + '","properties":' + str(json.dumps(hive_server2_xml_properties)) + ', "properties_attributes": {"final": "false"}},{"type": "ranger-hive-plugin-properties","tag":"' + str(date_time_stamp) + '", "properties":'+json.dumps(advanced_ranger_hive_plugin_properties)+',"properties_attributes": {"final": "false"}}]}}]'
...
def port_ranger_hbase_plugin_to_ambari():
  ...
  if flag_hbase_plugin_installed and hbase_plugin_install_properties is not None and hbase_site_xml_properties is not None:
    ...
    advanced_ranger_hbase_plugin_properties['XAAUDIT.HDFS.DESTINTATION_ROLLOVER_INTERVAL_SECONDS'] = hbase_plugin_install_properties['xasecure.audit.hdfs.config.destination.rollover.interval.seconds']
    advanced_ranger_hbase_plugin_properties['XAAUDIT.HDFS.DESTINTATION_OPEN_RETRY_INTERVAL_SECONDS'] = hbase_plugin_install_properties['xasecure.audit.hdfs.config.destination.open.retry.interval.seconds']
    ...
    advanced_ranger_hbase_plugin_properties['XAAUDIT.HDFS.LOCAL_BUFFER_ROLLOVER_INTERVAL_SECONDS'] = hbase_plugin_install_properties['xasecure.audit.hdfs.config.local.buffer.rollover.interval.seconds']
    ...
    advanced_ranger_hbase_plugin_properties['UPDATE_XAPOLICIES_ON_GRANT_REVOKE'] = hbase_plugin_install_properties['UPDATE_XAPOLICIES_ON_GRANT_REVOKE']

    date_time_stamp = getDateTimeNow()
    plugin_configuration_data = '[{"Clusters":{"desired_config":[{"type":"hbase-site","tag":"' + str(date_time_stamp) + '","properties":' + str(json.dumps(hbase_site_xml_properties)) + ', "properties_attributes": {"final": "false"}},{"type": "ranger-hbase-plugin-properties","tag":"' + str(date_time_stamp) + '", "properties":'+json.dumps(advanced_ranger_hbase_plugin_properties)+',"properties_attributes": {"final": "false"}}]}}]'
...
def port_ranger_knox_plugin_to_ambari():
  ...
  if flag_knox_plugin_installed and knox_plugin_install_properties is not None:
    ...
    advanced_ranger_knox_plugin_properties['XAAUDIT.HDFS.DESTINTATION_ROLLOVER_INTERVAL_SECONDS'] = knox_plugin_install_properties['xasecure.audit.hdfs.config.destination.rollover.interval.seconds']
    advanced_ranger_knox_plugin_properties['XAAUDIT.HDFS.DESTINTATION_OPEN_RETRY_INTERVAL_SECONDS'] = knox_plugin_install_properties['xasecure.audit.hdfs.config.destination.open.retry.interval.seconds']
    ...
    advanced_ranger_knox_plugin_properties['XAAUDIT.HDFS.LOCAL_BUFFER_ROLLOVER_INTERVAL_SECONDS'] = knox_plugin_install_properties['xasecure.audit.hdfs.config.local.buffer.rollover.interval.seconds']
    ...
    aliasValue = ''
    filepath = os.path.join('/etc/ranger' , knox_plugin_install_properties['REPOSITORY_NAME'],'cred.jceks')
    ...
    date_time_stamp = getDateTimeNow()
    plugin_configuration_data = '[{"Clusters":{"desired_config":[{"type": "ranger-knox-plugin-properties","tag":"' + str(date_time_stamp) + '", "properties":'+json.dumps(advanced_ranger_knox_plugin_properties)+',"properties_attributes": {"final": "false"}}]}}]'
...
def port_ranger_storm_plugin_to_ambari():
  ...
  if flag_storm_plugin_installed and storm_plugin_install_properties is not None:
    ...
    advanced_ranger_storm_plugin_properties['XAAUDIT.HDFS.DESTINTATION_ROLLOVER_INTERVAL_SECONDS'] = storm_plugin_install_properties['xasecure.audit.hdfs.config.destination.rollover.interval.seconds']
    advanced_ranger_storm_plugin_properties['XAAUDIT.HDFS.DESTINTATION_OPEN_RETRY_INTERVAL_SECONDS'] = storm_plugin_install_properties['xasecure.audit.hdfs.config.destination.open.retry.interval.seconds']
    ...
    advanced_ranger_storm_plugin_properties['XAAUDIT.HDFS.LOCAL_BUFFER_ROLLOVER_INTERVAL_SECONDS'] = storm_plugin_install_properties['xasecure.audit.hdfs.config.local.buffer.rollover.interval.seconds']
    ...
    date_time_stamp = getDateTimeNow()
    plugin_configuration_data = '[{"Clusters":{"desired_config":[{"type": "ranger-storm-plugin-properties","tag":"' + str(date_time_stamp) + '", "properties":'+json.dumps(advanced_ranger_storm_plugin_properties)+',"properties_attributes": {"final": "false"}}]}}]'
...
def get_hdfs_plugin_configuration():
  flag_hdfs_plugin_installed,hdfs_plugin_install_properties =  get_plugin_details('hdfs')
  if flag_hdfs_plugin_installed:
    ...
    hdfs_site_xml_properties = import_properties_from_xml(hdfs_site_xml_path, hdfs_plugin_install_properties)
    ...
    print('hdfs plugin is present but not installed to ranger not configuring plugin for the same.')
    return False,None,None

def get_hive_plugin_configuration():
  flag_hive_plugin_installed,hive_plugin_install_properties =  get_plugin_details('hive')
  if flag_hive_plugin_installed:
    ...
    hive_server2_xml_properties = import_properties_from_xml(hive_server2_xml_path, hive_plugin_install_properties)
...
def get_hbase_plugin_configuration():
  flag_hbase_plugin_installed,hbase_plugin_install_properties =  get_plugin_details('hbase')
  if flag_hbase_plugin_installed:
    ...
    hbase_site_xml_properties = import_properties_from_xml(hbase_site_xml_path, hbase_plugin_install_properties)
    ...
    return hbase_plugin_install_properties,hbase_plugin_install_properties,hbase_site_xml_properties
  else:
    print'hbase plugin is present and installed to ranger, configuring to setup in ambari.'
    return False,None,None
...
def get_knox_plugin_configuration():
  flag_knox_plugin_installed,knox_plugin_install_properties =  get_plugin_details('knox')
  if flag_knox_plugin_installed:
    ...
    return knox_plugin_install_properties,knox_plugin_install_properties
  else:
    print'knox plugin is present and installed to ranger, configuring to setup in ambari.'
    return False,None
...
def get_storm_plugin_configuration():
  flag_storm_plugin_installed,storm_plugin_install_properties =  get_plugin_details('storm')
  if flag_storm_plugin_installed:
    ...
    return storm_plugin_install_properties,storm_plugin_install_properties
  else:
    print'storm plugin is present and installed to ranger, configuring to setup in ambari.'
    return False,None
...
def get_plugin_details(component_name):
  print'getting ranger plugin details for ' + component_name
  flag_plugin_properties_present = False
  plugin_install_properties = dict()
  component_modified_name = 'ranger_' + component_name + '_plugin_install.properties'
  if os.path.isfile(component_modified_name):
    print component_name + ' is installed, reading properties'
    plugin_install_properties = import_properties_from_file(component_modified_name)
    if plugin_install_properties is not None and len(plugin_install_properties) != 0:
      print'plugin properties imported adding to existing configuration.'
      flag_plugin_installed =  check_plugin_enabled(component_name, plugin_install_properties)
      if flag_plugin_installed:
        flag_plugin_properties_present = True
      else:
        flag_plugin_properties_present = False
    else:
      print 'plugin properties is None hence setting flag to false'
      flag_plugin_properties_present = False
  else:
    print component_name + ' is not found installed not configuring properties for the same'
  return flag_plugin_properties_present,plugin_install_properties



def check_plugin_enabled(component_name,component_plugin_install_properties):
  flag_plugin_installed = False
  repo_base_path = os.path.join('/etc/ranger',component_plugin_install_properties['REPOSITORY_NAME'])
...
def call_ambari_api(ambari_url,method, username_password,data):
  try:
#     print('calling ambari api for given request')
#     print('ambari_url = ' + str(ambari_url))
#     print('method = ' + str(method))
#     print('username_password = ' + str(username_password))
#     print('data = ' + str(data))
    url = ambari_url    
