...
public class AuditRecoveryManager {
    ...
    public AuditRecoveryManager(Properties props, String propPrefix, AuditMessageQueue messageQueue, String topicName) {
        LOG.info("==> AuditRecoveryManager()");

        // Initialize writer and retry components
        // Pass messageQueue reference so retry can get current producer (may be recreated when Kafka comes online)
        this.recoveryWriter = new AuditRecoveryWriter(props, propPrefix);
        this.recoveryRetry  = new AuditRecoveryRetry(props, propPrefix, messageQueue, topicName);
