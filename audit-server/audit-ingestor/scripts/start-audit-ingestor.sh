...
# ========================================
# Ranger Audit Ingestor Service Start Script
# ========================================
# This script starts the Ranger Audit Ingestor service
...
# Find the WAR file
WAR_FILE=$(find "${AUDIT_SERVER_HOME_DIR}" -name "ranger-audit-ingestor*.war" | head -1)
...
WEBAPP_DIR="${AUDIT_SERVER_HOME_DIR}/webapp/ranger-audit-ingestor-service"
...
PID_FILE="${AUDIT_SERVER_LOG_DIR}/ranger-audit-ingestor.pid"
if [ -f "${PID_FILE}" ]; then
  OLD_PID=$(cat "${PID_FILE}")
  if kill -0 "$OLD_PID" 2>/dev/null; then
    echo "[WARNING] Ranger Audit Ingestor is already running (PID: ${OLD_PID})"
    echo "[INFO] Use stop-audit-ingestor.sh to stop it first"
    exit 1
  else
...
# Start the service
echo "[INFO] Starting Ranger Audit Ingestor Service..."
...
echo "[INFO] Ranger Audit Ingestor started successfully"
echo "[INFO] PID: ${PID}"
echo "[INFO] Log file: ${AUDIT_SERVER_LOG_DIR}/ranger-audit-ingestor.log"
echo "[INFO] Catalina out: ${AUDIT_SERVER_LOG_DIR}/catalina.out"
echo "[INFO] Health check: http://localhost:7081/api/audit/health"
echo ""
echo "To monitor logs: tail -f ${AUDIT_SERVER_LOG_DIR}/ranger-audit-ingestor.log"
echo "To stop service: ${SCRIPT_DIR}/stop-audit-ingestor.sh"
